...
export const GET: APIRoute = async () => {
  ...
  const { results } = await db.prepare(`
    SELECT l.slug, l.target_url, l.created_at, COUNT(c.id) as clicks, COUNT(DISTINCT c.visitor_hash) as unique_visitors
    FROM links l
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const body = (await request.json()) satisfies { target_url?: string; custom_slug?: string };
  ...
  try {
    await db.prepare(`INSERT INTO links (slug, target_url) VALUES (?, ?)`).bind(slug, targetUrl).run();
